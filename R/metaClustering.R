...
#  2021-1-22 fix bugs in kmax kmin kstep
#  2021-4-8  fix bugs in elbow test
...
MetaClustering <-
  function(data,
    ...
    if (elbow_test == T) {
      ...
      message(paste0("\n\nElbow test is finished,find best key_value:", res, "\n"))

    }
    method <- get(method)


    message(paste0("k_value is set to ", res, "\n"))
...
DetermineNumberOfClusters <- function(data,
  ...
  if (method == "metaClustering_PhenoGraph") {
    ...
    for (i in 1:length(kseq)) {
      ...
      c <-
        as.numeric(igraph::membership(cytofkit::Rphenograph(data, k = kseq[i])))
    ...
    if (plot) {
      ...
      plot(
        ...
        main = paste0(method, " Elbow Test")
      )
      points(kseq[Elbowpoint2],
             nclus[Elbowpoint2],
             pch = 3,
             col = "blue",
             cex = 2)
  ...
  if (method == "metaClustering_consensus") {
    ...
    if (!is.null(kmin) |
        !is.null(kstep)) {
      cat("kmin and kstep can only set to 1 when clustering with metaClustering_consensus ")
    }
  ...
  for (i in 2:(length(kseq) - 1)) {
    res[i] <- (1 - smooth) * res[i] + (smooth / 2) * res[i - 1] + (smooth /
                                                                     2) * res[i + 1]
  }
  ...
  if (plot) {
    plot(
      ...
      main = paste0(methodname, " Elbow Test")
    )
    points(kseq[Elbowpoint],
           res[Elbowpoint],
           pch = 3,
           col = "blue",
           cex = 2)
...
metaClustering_PhenoGraph <- function(data, k = 30) {
  as.numeric(igraph::membership(cytofkit::Rphenograph(data, k = k)))
...
MetaclusterMFIs <- function(fsom) {
  MFIs <- t(sapply(seq_along(levels(
    fsom$metaclustering
  )),
  function(i) {
...
metaClustering <- function(indataframe = NULL,
  ...
  cat("MetaClustering using method:",
      metaClustering_method,
      "...\n")
  if (elbow_test == T) {
  ...
  Cluster_arrange <-
    data.frame(cluster = cluster_center_expr[, clustername],
               metacluster = cc_metacluster)

  cluster_arrange_fun <- function(cluster_id) {
    cellcluster <-
      subset(Cluster_arrange, Cluster_arrange[, 1] == cluster_id)$metacluster
    return(cellcluster)
  }
  metacluster_result <-
    apply(as.matrix(indataframe[, colnames(indataframe) == clustername]), 1, cluster_arrange_fun)
  ...
  if (view_tsne == T)
  {
    ...
    combine_data_plot$metacluster <-
      as.factor(combine_data_plot$metacluster)
...
#
#     klab="Cluster number(k_value)"
#     if(elbow_test)k_value="auto-find"
#     cat(elbow_test)
...
map_to_singlecells <-
...

  {
    ...
    metacluster_result <-
      apply(as.matrix(indataframe[, colnames(indataframe) == "FlowSOM"]), 1, cluster_arrange_fun)
